...
module.exports = (rideHistoryService) => {
  ...
  router.post('/:rideId/start', async (req, res) => {
    try {
      ...
      const result = await rideHistoryService.startRide(rideId, driverPhone);
      res.json(result);
    } catch (error) {
      res.status(500).json({ success: false, error: error.message });
    }
  ...
  router.post('/:rideId/complete', async (req, res) => {
    try {
      const { rideId } = req.params;
      const { driverPhone, completionData } = req.body;
      
      const result = await rideHistoryService.completeRide(rideId, driverPhone, completionData);
      res.json(result);
    } catch (error) {
      res.status(500).json({ success: false, error: error.message });
    }
  ...
  router.post('/:rideId/cancel', async (req, res) => {
    try {
      const { rideId } = req.params;
      const { cancelledBy, reason, details } = req.body;
      
      const result = await rideHistoryService.cancelRide(rideId, cancelledBy, reason, details);
      res.json(result);
    } catch (error) {
      res.status(500).json({ success: false, error: error.message });
    }
