// moving_passenger_simulation.js - UPDATED FOR DYNAMIC PICKUP UPDATES
...
class MovingPassengerSimulation {
  constructor() {
    ...
    // 🎯 ORIGINAL PICKUP LOCATION (never changes - initial starting point)
  ...
  // 🎯 NEW: Get REMAINING route points from current position to destination
  getRemainingRoutePoints() {
    if (this.currentRouteIndex >= this.ROUTE_POINTS.length) {
      return this.ROUTE_POINTS.map(p => ({ lat: p.lat, lng: p.lng }));
    }
    
    // Return points from current position to destination
    return this.ROUTE_POINTS.slice(this.currentRouteIndex).map(p => ({
      lat: p.lat,
      lng: p.lng
    }));
  }

  // 🎯 NEW: Calculate distance from original pickup
  getDistanceFromOriginalPickup() {
    if (this.currentRouteIndex === 0) return 0;
    
    const currentPoint = this.ROUTE_POINTS[this.currentRouteIndex];
    const dx = currentPoint.lat - this.ORIGINAL_PICKUP.lat;
    const dy = currentPoint.lng - this.ORIGINAL_PICKUP.lng;
    
    // Approximate distance in km (simplified for testing)
  ...
  async startPassengerSearch() {
    try {
      ...
      console.log('PICKUP LOCATION WILL UPDATE TO CURRENT POSITION WITH EACH MOVE');
      ...
      const searchData = {
        ...
        // 🎯 ORIGINAL PICKUP LOCATION (starting point)
        pickupLocation: {
        ...
        specialRequests: "Moving passenger test - pickup updates with location",
        maxWalkDistance: 0.5,
        distance: 320,
        duration: 360,
        estimatedFare: 800,
        ...
        // 🎯 NEW: Flag this as a moving passenger
        isMovingPassenger: true,
        dynamicPickup: true
      };
      ...
      console.log('Initial pickup: (' + 
        this.ORIGINAL_PICKUP.lat + ', ' + this.ORIGINAL_PICKUP.lng + ')');
      console.log('Will update pickup location with each movement');
      ...
      if (response.statusCode === 200 && response.data.success) {
        ...
        console.log('   Dynamic Pickup: ENABLED');
  ...
  startMovementUpdates() {
    ...
    console.log('   PICKUP LOCATION WILL UPDATE WITH EACH MOVE');
    ...
    this.movementInterval = setInterval(() => {
      if (this.isSearching) {
        ...
        if (moveResult.moved) {
          ...
          // Calculate distance from original pickup
          const distanceFromOriginal = this.getDistanceFromOriginalPickup();
          console.log('   Distance from original pickup: ' + distanceFromOriginal.toFixed(1) + ' km');
  ...
  async sendLocationUpdate() {
    try {
      ...
      const remainingRoutePoints = this.getRemainingRoutePoints();
      const distanceFromOriginal = this.getDistanceFromOriginalPickup();
      ...
      console.log('   Distance from original pickup: ' + distanceFromOriginal.toFixed(1) + ' km');
      console.log('   Remaining route points: ' + remainingRoutePoints.length);
      
      // 🎯 CRITICAL: Prepare updated pickup location data
      const updatedPickupLocation = {
        latitude: currentLocation.latitude,
        longitude: currentLocation.longitude
      };
      
      const updatedPickupName = `Current Location (${this.ORIGINAL_PICKUP.name}) - Progress: ${Math.round((this.currentRouteIndex / this.ROUTE_POINTS.length) * 100)}%`;
      
      // 🎯 OPTION 1: Use the standard endpoint with enhanced data
      const locationData = {
        userId: this.passengerId,
        ...
        // 🎯 CRITICAL: Tell backend to update pickup location
        updatePickupLocation: true, // Flag to trigger pickup update
        pickupLocation: updatedPickupLocation, // New pickup position
        pickupName: updatedPickupName, // Updated pickup name
        
        // 🎯 CRITICAL: Send updated route points
        routePoints: remainingRoutePoints, // Route from current position to destination
        ...
        distanceFromOriginalPickup: distanceFromOriginal,
        isMovingPassenger: true
      };
      
      // 🎯 OPTION 2: Try enhanced endpoint first, fall back to standard
      let endpoint = '/api/passenger/update-location';
      
      // First try enhanced endpoint if it exists
      const enhancedOptions = {
        hostname: this.CONFIG.baseUrl,
        port: this.CONFIG.port,
        path: '/api/passenger/update-moving-location',
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json'
      ...
      console.log('   New pickup location: (' + 
        updatedPickupLocation.latitude.toFixed(6) + ', ' + 
        updatedPickupLocation.longitude.toFixed(6) + ')');
      console.log('   Sending ' + remainingRoutePoints.length + ' remaining route points');
      
      // Try enhanced endpoint first
      let response;
      try {
        response = await this.makeRequest(enhancedOptions, locationData);
        console.log('   Using enhanced moving passenger endpoint');
      } catch (error) {
        // Fall back to standard endpoint
        console.log('   Enhanced endpoint not available, using standard endpoint');
        ...
        response = await this.makeRequest(standardOptions, locationData);
      }
      
      if (response.statusCode === 200 && response.data.success) {
        console.log('✅ Location update successful!');
        console.log('   Pickup updated: ' + (response.data.pickupUpdated ? 'YES' : 'NO'));
        console.log('   Route updated: ' + (response.data.routeUpdated ? 'YES' : 'NO'));
  ...
  // 🎯 NEW: Send test command to update pickup location manually
  async forceUpdatePickupLocation() {
    try {
      const currentLocation = this.getCurrentLocation();
      ...
      const forceUpdateData = {
        ...
        pickupName: `Forced Update: Current Location (${this.ORIGINAL_PICKUP.name})`,
        forceUpdate: true
      };
      ...
      console.log('Forcing pickup location update...');
      const response = await this.makeRequest(options, forceUpdateData);
      
      if (response.statusCode === 200) {
        console.log('✅ Forced pickup update successful');
        return true;
      }
      
    } catch (error) {
      console.log('Force update failed (endpoint may not exist): ' + error.message);
    }
    return false;
  }
  ...
  async healthCheck() {
    try {
      ...
      if (response.statusCode === 200 && response.data.success) {
        ...
        console.log('   Dynamic Route Updates: ' + 
          (response.data.features?.dynamicRouteUpdates ? 'Enabled' : 'Disabled'));
        return true;
      }
      return false;
    } catch (error) {
  ...
  // 🎯 NEW: Check search status to verify pickup was updated
  async checkSearchStatus() {
    try {
      const options = {
        hostname: this.CONFIG.baseUrl,
        port: this.CONFIG.port,
        path: '/api/match/search-status/' + this.passengerId,
        method: 'GET',
        headers: {
          'Accept': 'application/json'
        }
      ...
      if (response.statusCode === 200 && response.data.success) {
        ...
        console.log('   Search Active: ' + (response.data.memorySearch?.exists ? 'YES' : 'NO'));
        
        if (response.data.memorySearch?.exists) {
          console.log('   Current Pickup: ' + 
            (response.data.memorySearch.pickupName || 'Unknown'));
          console.log('   Matches Found: ' + response.data.matches?.count);
        }
  ...
  async runSimulation() {
    ...
    setTimeout(async () => {
      console.log('\n=== FIRST STATUS CHECK (30 seconds) ===');
      await this.checkSearchStatus();
    }, 30000);
    
    // Check status halfway through
    setTimeout(async () => {
      console.log('\n=== MID-SIMULATION STATUS CHECK ===');
      await this.checkSearchStatus();
    }, this.CONFIG.testDuration / 2);
  ...
  async printStatistics() {
    ...
    console.log('   Final Route Progress: ' + 
      Math.round((this.currentRouteIndex / this.ROUTE_POINTS.length) * 100) + '%');
    console.log('   Distance Traveled from Original: ' + 
      this.getDistanceFromOriginalPickup().toFixed(1) + ' km');
    console.log('   WebSocket Connected: ' + (this.websocketConnected ? 'Yes' : 'No'));
    console.log('   Pickup Updates Sent: ' + this.locationUpdates + ' (with each location update)');
  }
}
